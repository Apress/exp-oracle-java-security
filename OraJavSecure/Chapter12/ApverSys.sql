...
-- Grant to user, not to role
GRANT EXECUTE ON sys.usr_role_adm_pkg TO ojsaadm;
...
-- This is already inserted on orcl instance
...
-- Note: after this trigger, cannot insert on apver if already on orcl
-- possibly some error handling in trigger would help
...
-- You might make sure this was done in Chapter 11
...
ALTER USER appsec IDENTIFIED BY password;

-- This test query needs to run as appsec to use private database link
